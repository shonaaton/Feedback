...
function groupCoachPending(tasks) {
  const groups = new Map();
  tasks.forEach((task) => {
    const key = normalizeEmail(task.coachEmail) || String(task.coachName || 'Unassigned Coach').trim().toLowerCase();
    if (!groups.has(key)) {
      groups.set(key, {
        coachEmail: normalizeEmail(task.coachEmail),
        coachName: task.coachName || task.coachEmail || 'Unassigned Coach',
        pendingCount: 0,
        students: []
      });
    }
...
async function getMentorDashboard(db, { month, mentorEmail, isAdmin = false }) {
  ...
  const coachPendingTasks = tasks.filter(isCoachPending);
  return {
    ...
    coachPendingTasks,
    coachPendingGroups: groupCoachPending(coachPendingTasks)
  };
}
...
async function getApprovedDashboard(db, { month, mentorEmail, isAdmin = false }) {
  ...
  const legacyTasks = (await getLegacyTasksByMonth(db, month)).filter((task) => {
    const matchesMentor = isAdmin || !task.mentorEmail || task.mentorEmail === normalizeEmail(mentorEmail);
    return matchesMentor && isApproved(task);
  });
  const deduped = new Map();
  [...data.completedTasks, ...legacyTasks].forEach((task) => {
    const key = [
      monthKey(task.monthKey || task.month || ''),
      String(task.studentId || '').trim().toLowerCase(),
      normalizeEmail(task.coachEmail),
      normalizeEmail(task.mentorEmail)
    ].join('|');
    if (!deduped.has(key) || !task.legacy) {
      deduped.set(key, task);
    }
  });
  const tasks = Array.from(deduped.values()).filter(isApproved).sort((left, right) => {
