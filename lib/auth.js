...
function rolesMatch(left, right) {
  ...
  if (a === b) return true;
  const sharedAdminLane = ['mentor', 'admin'];
  return sharedAdminLane.includes(a) && sharedAdminLane.includes(b);
}
...
async function verifyOtpAndCreateSession(db, { email, role, code }) {
  ...
  const otpCandidates = await db.collection('otpCodes').find(
    {
  ...
  const otp = otpCandidates.find((candidate) => rolesMatch(candidate.role, normalizedRole));
  ...
  const session = {
    ...
    email: normalizedEmail,
    role: normalizeRole((user && user.role) || normalizedRole),
    name: user && user.name ? String(user.name).trim() : '',
